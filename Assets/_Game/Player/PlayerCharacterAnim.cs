...
namespace SurvivalEngine
{
    ...
    public class PlayerCharacterAnim : MonoBehaviour
    {
        ...
        void Update()
        {
            bool player_paused = false;// TheGame.Get().IsPausedByPlayer();
            bool gameplay_paused = false;// TheGame.Get().IsPausedByScript();
            _animator.enabled = !player_paused;

            if (_animator.enabled)
            {
                SetAnimBool(move_anim, !gameplay_paused && _controller.IsMoving);
